...
public class CameraMove : MonoBehaviour
{
    ...
    void Start()
    {
        target = GameObject.Find("Direction");
        follow = target.GetComponent<Transform>();
    }
    private void LateUpdate()
    { 
        transform.position = new Vector3(transform.position.x, transform.position.y, -10f);
        transform.position = Vector3.MoveTowards(transform.position, follow.position, 10 * Time.deltaTime);
    }
}
/*
